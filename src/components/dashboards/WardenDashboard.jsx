...
const WardenDashboard = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [studentsRes, roomsRes, complaintsRes] = await Promise.all([
                axios.get('getApiUrl(' / warden / students', { headers }),
                axios.get('getApiUrl(' / warden / rooms', { headers }),
                axios.get('getApiUrl(' / warden / complaints', { headers })
            ]);
    ...
    const handleAddStudent = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post('getApiUrl(' / warden / students', studentForm, {
...
const handleAddRoom = async (e) => {
    ...
    try {
        const token = localStorage.getItem('token');
        await axios.post('getApiUrl(' / warden / rooms', roomForm, {
                headers: { Authorization: `Bearer ${token}` }
            });
...
const updateComplaintStatus = async (complaintId, status, response = '') => {
    try {
        await axios.put(`getApiUrl('/warden/complaints/${complaintId}`, {
            status,
